...
# =============================================================================
GRAFT = "toy2"  # kept only to minimize downstream naming changes
...
# 3-body toy term (angle-like) + exponential envelope
THETA0_DEG = 100.0
...
def y_dimer_basis(d, k=None, x0=None):
    return lj_12_6(d)

def y_trimer_basis(d, k=None, x0=None):
    d = np.maximum(np.asarray(d, float), 1e-12)
    return np.exp(-GAMMA_3B*(d - R0_3B))/d

def W2(d, p2=None):
    return lj_12_6(d)

def toy_three_body(d12, d13, d23,
                   lam=LAMBDA_3B, cos_th0=COS_TH0, gamma=GAMMA_3B, r0=R0_3B):
...
def sf_sum(d12, d13, d23, p3=None):
    return np.ones_like(np.asarray(d12, float))
...
# INIT_EDGE_COUNT       = 2
# INIT_FACE_DIAG_COUNT  = 0
# INIT_SPACE_DIAG_COUNT = 0
...
def make_train_xy_from_nodes(train_nodes, y_phys, Xs_feas, feas_pos):
    """
    CRITICAL: sanitize X and physical y together to avoid shape mismatch.
    Returns: X_tr, y_tr_phys, z_tr, ymu, ysd
    """
...
def main():
    ...
    plot_scatter3d_color(
        ...
        title=rf"Toy oracle $U_{{\mathrm{{mb}}}}$ (anchored), {GRAFT}",
    ...
    plot_face_2d_heat(d13f, d23f, Uface_a, maskf, "Umb_01_oracle_Umb_face_d12min_slice2D",
        title=rf"Toy oracle $U_{{\mathrm{{mb}}}}$ face ($d_{{12}}={D12_MIN:.2f}$) (anchored), {GRAFT}",
        clabel=LAB_C, cmap="viridis", norm=normU
    )
    plot_face_3d_surface(D13f, D23f, Uface_a, maskf, "Umb_01_oracle_Umb_face_d12min_slice3D",
        title=rf"Toy oracle $U_{{\mathrm{{mb}}}}$ face surface ($d_{{12}}={D12_MIN:.2f}$) (anchored), {GRAFT}",
        clabel=LAB_C, cmap="viridis", norm=normU
    )
    ...
    plot_scatter3d_color(P_full[plot_g], dW3_flat[plot_g], LAB_DW3,
        ...
        title=rf"Toy oracle $\Delta W_3$, {GRAFT}",
        cmap="viridis", norm=None, scatter_swap_xy=False
    )

    # ===== Optional LSQR teacher force machinery =====
    U_full_map = None
    Fx = Fy = Fz = None

    def teacher_query_force_at_nodes(_nodes):
        return  # no-op unless LSQR is enabled

    if USE_LSQR:
        Fd12, Fd13, Fd23 = forces_Umb(D12v, D13v, D23v)
        Fd12 = np.where(feas, Fd12, np.nan)
        Fd13 = np.where(feas, Fd13, np.nan)
        Fd23 = np.where(feas, Fd23, np.nan)

        Fd12_flat = Fd12.ravel(order="C")
        Fd13_flat = Fd13.ravel(order="C")
        Fd23_flat = Fd23.ravel(order="C")
        ...
        if DO_FULLGRID_LSQR_DIAGNOSTIC:
            ...
            if PLOT_FULLGRID_LSQR_DIAGNOSTIC:
                plot_scatter3d_color(
                    ...
                    title=rf"LSQR PMF from toy oracle forces (full-grid diagnostic), {GRAFT}",
                    cmap="viridis", norm=normU, scatter_swap_xy=False
                )
            Fx[:] = np.nan; Fy[:] = np.nan; Fz[:] = np.nan

    # ===== Permutation-invariant GP coordinates on feasible set =====
    ...
    # Val nodes are forbidden from being paid (path paying) or trained on
    allowed_train_mask = feas_flat & (~is_val)
    allowed_train_mask[int(anchor_g)] = True

    # Val (oracle) targets + standardized val X
    ...
    def _build_training_nodes_and_labels(label_source, train_all, train_q,
        ...
        """
        Returns:
          train_nodes : nodes used for TRAINING labels (depends on label_source)
          y_phys      : physical labels aligned with train_nodes (Umb or Residual)
          paid_nodes_lsqr, U_paid_lsqr : for optional LSQR parity figure (only if lsqr)
        """
    ...
    # Hard rules: val never overlaps train_all/train_q
    ...
    # ===== min-sep from standardized (perm-invariant) space =====
    ...
    for it in range(AL_ITERS):
        # Candidates exclude PAID nodes and val nodes
        ...
        # Model uncertainty on feasible set
        _, std_old_z = predict_mu_std_z(prev_m, Xs_feas)

        # Distance gating relative to PAID set (train_all)
        ...
        # Info gain proxy (in z-space) + spacing bonus
        ...
        # Diversity pick in feature space
        take_g = []
        taken = []
        r_min = base_h
        for idx in order:
        ...
        # Candidate overlay neighborhood for plotting
        cand_overlay = set()
        ...
        # Plot previous accepted model before updating sets
        ...
        # Path-paying to keep connectivity, but NEVER pay val nodes
        ...
        # Build training labels per label_source
        train_nodes, y_phys_raw, paid_nodes_lsqr, U_paid_lsqr = _build_training_nodes_and_labels(
            LABEL_SOURCE, train_all, train_q,
            anchor_g, P_full, n12, n13, n23, feas_flat,
            Fx, Fy, Fz,
        ...
        # Build X_tr/y_tr/z_tr + scaling for THIS step
        X_tr, y_tr, z_tr, ymu, ysd = make_train_xy_from_nodes(train_nodes, y_phys_raw, Xs_feas, feas_pos)
        if X_tr.shape[0] < 2:
            print(f"[AL {it+1:02d}] WARNING: too few points after sanitization; skipping update.")
            continue

        # Evaluate previous model on current train/val (with its own scaling)
